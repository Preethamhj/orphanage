        ...
        child: FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            final cards = <({String title, String value, IconData icon})>[
              (title: 'Total Children', value: '${stats['children'] ?? 0}', icon: Icons.child_care_outlined),
              (title: 'Total Staff', value: '${stats['staff'] ?? 0}', icon: Icons.badge_outlined),
              (title: 'Total Donations', value: '${stats['donations'] ?? 0}', icon: Icons.volunteer_activism_outlined),
              (title: 'Total Adoptions', value: '${stats['adoptions'] ?? 0}', icon: Icons.favorite_outline),
              (title: 'Donation Amount', value: '${stats['totalDonationAmount'] ?? 0}', icon: Icons.currency_rupee),
            ];
                ...
                return Scrollbar(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      const SizedBox(height: 14),
                      const Text('Quick Links', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 8),
                      links,
                      const SizedBox(height: 14),
                      const Text('Recent Logins', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 8),
                      Card(
                        child: Padding(
                              ...
                              : Column(
                                  children: _recentLogins
                                      .map(
                                        (e) => ListTile(
                                          dense: true,
                                          title: Text('${e['email']} (${e['role']})'),
                                          subtitle: Text((e['login_time'] ?? '').toString()),
                                        ),
                                      )
                                      .toList(),
                                ),
                      ...
                      const SizedBox(height: 14),
                      const Text('Pending User Approvals', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 8),
                      Card(
                        child: Padding(
                          padding: const EdgeInsets.all(10),
                              ...
                              : Column(
                                  children: _pendingUsers
                                      .map(
                                        (u) => ListTile(
                                          dense: true,
                                          title: Text('${u['email']} (${u['role']})'),
                                          subtitle: Text((u['full_name'] ?? '').toString()),
                                          trailing: TextButton(
                                            onPressed: () => _approveUser((u['id'] ?? '').toString()),
